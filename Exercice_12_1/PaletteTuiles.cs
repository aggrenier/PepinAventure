...
namespace IFM20884
{
    ...
    public class PaletteTuiles
    {
        ...
        /// <summary>
        /// Retourne une liste des différentes couleurs retrouvées dans la tuile indiquée.
        /// </summary>
        /// <param name="tuileIdx">Index de tuile à considérer.</param>
        /// <param name="couleurs">Liste des différentes couleurs de pixels trouvés dans la tuile.</param>
        /// <returns>Nombre de couleurs différentes trouvées dans la tuile.</returns>
        public int CouleursDansTuile(int tuileIdx, out List<Color> couleurs)
        {
            // Calculer le nombre de tuiles dans une rangée de la palette.
            int tuilesParRangee = this.tuiles.Width / this.LargeurTuile;
            ...
            // Créer un tableau de couleurs où seront stockés les couleurs des pixels de la tuile.
            Color[] colorData = new Color[this.LargeurTuile * this.HauteurTuile];

            // Calculer un rectangle correspondant à la tuile dans la palette.
            ...
            // Extraire les couleurs de la tuile.
            this.tuiles.GetData<Color>(0, targetRect, colorData, 0, this.LargeurTuile * this.HauteurTuile);

            // Constituer une liste des couleurs distinctes retrouvées dans la tuile.
            couleurs = new List<Color>();
            foreach (Color clr in colorData)
            {
                if (!couleurs.Exists(delegate(Color c) { return c == clr; }))
                {
                    couleurs.Add(clr);
                }
            }
